...
const (
	...
	//MAX_ALLOWED_TIMESERIES_ID = 10000000 // remove to bucketMap
...
func (d *DataLogWriter) Append(id uint32, unixTime int64, value float64) error {
	...
	if xorWithPrevious == 0 {
		...
		leadingZeros := Clz(xorWithPrevious)
		trailingZeros := Ctz(xorWithPrevious)
		if leadingZeros > 31 {
			leadingZeros = 31
		}
...
func ReadLog(file *File, baseTime int64,
