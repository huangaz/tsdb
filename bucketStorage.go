...
const (
	...
	//PAGE_SIZE uint32 = DATA_BLOCK_SIZE // remove to dataTypes
...
type BucketStorage struct {
	...
	data_            []*BucketData
	dataBlockReader_ *DataBlockReader
	dataFiles_       *FileUtils
	completeFiles_   *FileUtils
}
...
func (b *BucketStorage) write(position, activePages uint32, pages []*DataBlock,
	...
	dataLen := 4 + 4 + 4*count + 8*count + int(activePages)*DATA_BLOCK_SIZE
	...
	for i := uint32(0); i < activePages; i++ {
		...
		binary.Read(bytes.NewReader(tmpSlice), binary.BigEndian,
			buffer[index:index+DATA_BLOCK_SIZE])
